...
#SBATCH --output="/users/k21157612/ukb_rap/temp/ad_switch/job_logs/%j_gcta_SSRI_switch_all"
...
--keep $PHENO_TEMP"/id_switch_SSRI_all_noheader.txt" \
--extract $SGDP_QC"ukb82087_post_qc_snp_list_relatives_included.txt" \
--out $OUT"/SSRI_all_gcta"" 
...
cat SSRI_all_gcta.part_10_*.grm.id > SSRI_all_gcta.grm.id
cat SSRI_all_gcta.part_10_*.grm.bin > SSRI_all_gcta.grm.bin
cat SSRI_all_gcta.part_10_*.grm.N.bin > SSRI_all_gcta.grm.N.bin

# remove the subsets:
rm SSRI_all_gcta.part_10*
...
--grm $OUT"/SSRI_all_gcta" \
--grm-adj 0 \
--make-grm \
--out $OUT"/SSRI_all_gcta.adjusted""

# Remove related subjects using grm-cutoff 0.05:
remove_rel="$SOFTWARE"/gcta-1.94.1" \
--grm $OUT"/SSRI_all_gcta.adjusted" \
--grm-cutoff 0.05 \
--make-grm \
--out $OUT"/SSRI_all_gcta.adjusted.unrel""

# Run GREML-SC:
greml="$SOFTWARE"/gcta-1.94.1" \
--grm $OUT"/SSRI_all_gcta.adjusted.unrel" \
--reml \
--reml-no-constrain \
--pheno $PHENO_TEMP"/id_switch_SSRI_all_noheader.txt" \
--covar $COV"/cov_switch_SSRI_all_cat_noheader.txt" \
--qcovar $COV"/cov_switch_SSRI_all_num_noheader.txt" \
--out $OUT"/SSRI_all_gcta.adjust.unrel.GREML_SC""
