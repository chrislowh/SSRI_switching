...
#SBATCH --output="/users/k21157612/ukb_rap/temp/ad_switch/job_logs/%j_gcta_ssri_switch_onemdd"
...
--keep $PHENO"/id_switch_SSRI_onemdd_noheader.txt" \
--extract $SGDP_QC"ukb82087_post_qc_snp_list_relatives_included.txt" \
--out $OUT"/SSRI_onemdd_gcta"" 
...
cat SSRI_onemdd_gcta.part_10_*.grm.id > SSRI_onemdd_gcta.grm.id
cat SSRI_onemdd_gcta.part_10_*.grm.bin > SSRI_onemdd_gcta.grm.bin
cat SSRI_onemdd_gcta.part_10_*.grm.N.bin > SSRI_onemdd_gcta.grm.N.bin

# remove the subsets:
rm SSRI_onemdd_gcta.part_10*
...
--grm $OUT"/SSRI_onemdd_gcta" \
--grm-adj 0 \
--make-grm \
--out $OUT"/SSRI_onemdd_gcta.adjusted""

# Remove related subjects using grm-cutoff 0.05:
remove_rel="$SOFTWARE"/gcta-1.94.1" \
--grm $OUT"/SSRI_onemdd_gcta.adjusted" \
--grm-cutoff 0.05 \
--make-grm \
--out $OUT"/SSRI_onemdd_gcta.adjusted.unrel""

# Run GREML-SC:
greml="$SOFTWARE"/gcta-1.94.1" \
--grm $OUT"/SSRI_onemdd_gcta.adjusted.unrel" \
--reml \
--reml-no-constrain \
--pheno $PHENO"/id_switch_SSRI_onemdd_noheader.txt" \
--covar $COV"/cov_switch_SSRI_onemdd_cat_noheader.txt" \
--qcovar $COV"/cov_switch_SSRI_onemdd_num_noheader.txt" \
--out $OUT"/SSRI_onemdd_gcta.adjust.unrel.GREML_SC""
